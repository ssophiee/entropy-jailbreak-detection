...
def main(args):
    log.info("=" * 80)
    log.info("ENSEMBLE LORA — ENTROPY TRACE METRICS")
    log.info("=" * 80)
    log.info("Device          : %s", constants.DEVICE)
    log.info("Base Model      : %s", constants.MODEL_NAME)
    log.info("Ensemble Dir    : %s", args.ensemble_dir)
    ...
    # 2) Load ensemble
    log.info("[2/4] Loading ensemble from %s...", args.ensemble_dir)
    ensemble = load_ensemble_from_directory(
        base_model_name=constants.MODEL_NAME,
        ensemble_dir=args.ensemble_dir,
        device=constants.DEVICE,
    )
    log.info("  Adapters: %d", len(ensemble.adapter_paths))

    # 3) Compute entropy trace features
    log.info("[3/4] Computing entropy trace features...")
